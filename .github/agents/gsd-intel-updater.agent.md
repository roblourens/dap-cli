...
if [[ "$(jq -r '.name // ""' package.json 2>/dev/null)" == "get-shit-done-redux" ]]; then
  ls -d .kilo 2>/dev/null && echo "kilo" || (ls -d .github/get-shit-done 2>/dev/null && echo "claude") || echo "unknown"
fi
```
...
counts by running Glob on the layout-resolved canonical locations above, not from memory or copilot-instructions.md.
