...
@.github/get-shit-done/references/few-shot-examples/plan-checker.md
...
**Question:** Do plans respect project-specific conventions, constraints, and requirements from copilot-instructions.md?

**Process:**
1. Read `./copilot-instructions.md` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns copilot-instructions.md explicitly forbids
5. Flag plans that skip steps copilot-instructions.md explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern copilot-instructions.md explicitly forbids
- Plan skips a required step (e.g., copilot-instructions.md says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts copilot-instructions.md conventions
- Plan creates files in locations that violate copilot-instructions.md's architectural constraints
- Plan ignores security requirements documented in copilot-instructions.md

**Skip condition:** If no `./copilot-instructions.md` exists in the working directory, output: "Dimension 10: SKIPPED (no copilot-instructions.md found)" and move on.
...
issue:
  ...
  severity: blocker
  description: "Plan uses Jest for testing but copilot-instructions.md requires Vitest"
  plan: "01"
  task: 1
  claude_md_rule: "Testing: Always use Vitest, never Jest"
  plan_action: "Install Jest and create test suite..."
  fix_hint: "Replace Jest with Vitest per project copilot-instructions.md"
```
...
issue:
  dimension: claude_md_compliance
  severity: warning
  description: "Plan does not include lint step required by copilot-instructions.md"
  plan: "02"
...
- [ ] copilot-instructions.md compliance checked (plans respect project conventions)
