...
**Project instructions:** Read `./copilot-instructions.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during fixes.
...
Read `./copilot-instructions.md` and check for `.github/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO respect copilot-instructions.md project conventions** during fixes. If project requires specific patterns (e.g., no `any` types, specific error handling), apply them.
...
- [ ] Project conventions from copilot-instructions.md respected during fixes
