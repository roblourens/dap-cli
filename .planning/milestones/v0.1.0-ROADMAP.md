# Milestone v0.1.0: dap-cli initial public release

**Status:** ✅ SHIPPED 2026-05-12
**Phases:** 1–19 (21 phases including decimals 5.1, 5.2)
**Total Plans:** 91
**Total Commits:** 45 (4047ede → e383e05)
**Timeline:** 2026-05-02 → 2026-05-12 (10 days)
**LOC (TS, src + tests):** ~24,000
**npm version:** `0.1.0`
**Audit:** [.planning/v0.1.0-MILESTONE-AUDIT.md](../v0.1.0-MILESTONE-AUDIT.md) — passed (34/34 requirements)

## Overview

v0.1.0 builds dap-cli from a clean TypeScript/Node foundation into a published
agent-facing Debug Adapter Protocol CLI. Persistent controller, vanilla
language-neutral DAP core, generated typed command surface for every DAP
request, built-in JS (js-debug) + Python (debugpy) adapters with custom adapter
support, real Chrome/js-debug/Playwright same-browser handoff, VS Code
launch.json + compound execution, paused-state ergonomics with per-child
routing, breakpoint command surface, JSON-by-default-when-piped output mode,
human-readable output mode, Python evaluate auto-wrap, two hardening
campaigns (internal + external projects), and a categorized help command.

## Key Accomplishments

1. **Persistent controller + modular architecture + language-neutral DAP core** with stdio + socket transports, deterministic fake-adapter tests (Phase 1).
2. **Complete typed DAP command surface** generated from official protocol metadata, with raw JSON passthrough escape hatch and capability reporting (Phase 2).
3. **Built-in JS (js-debug) + Python (debugpy) + custom adapter support**, source-map smoke coverage, and adapter-override flows (Phase 3).
4. **Real Chrome/js-debug + Playwright same-browser handoff** with child-session enumeration and event routing for js-debug pwa-* hierarchy (Phase 5, 15).
5. **VS Code launch.json + compound execution** with variable substitution, platform overrides, AUTOROUTE-01 launch↔attach detection, and `--json-overrides` merge layer (Phase 5.2, 10).
6. **Human-readable output mode** + auto-JSON when stdout is not a TTY, dropping the `--no-human` workaround for agent pipelines (Phase 5.1, 13).
7. **Paused-state ergonomics**: status reflects stopped/continued events, evaluate auto-uses topmost paused frame, per-child paused state with paused-first routing under multi-child js-debug (Phase 11, 18).
8. **Breakpoint command surface**: `breakpoints set/list/clear` with verification diagnostics and loaded-sources hints (Phase 12).
9. **Python evaluate ergonomics**: auto-wrap statements as `exec(...)` with documented `no-auto-wrap` opt-out (Phase 16).
10. **Two hardening campaigns** (internal + external real-repo projects) with all discovered gaps closed inline as GSD UAT items (Phase 7, 8, 17).
11. **Categorized help command** with seven locked headings and drill-down for subcommands (Phase 19).
12. **Agent workflow docs + Playwright interop coverage** including conditional breakpoints (Phase 4, 6, 14).
