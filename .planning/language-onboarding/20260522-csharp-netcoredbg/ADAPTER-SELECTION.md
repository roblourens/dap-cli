...
| --- | --- | --- | --- |
| C#/.NET + NetCoreDbg | Strong DAP fit; standalone `--interpreter=vscode`; launch and attach model maps to dap-cli; MIT license; GitHub release assets include platform binaries and SHA-256 digests; .NET SDK containers make verification practical. | Not Microsoft-owned; macOS arm64 quality should be verified because upstream docs call out some community-support caveats; dap-cli must define a clear build-before-debug convention for `.csproj` targets. | **Recommended** |
| C#/.NET + Microsoft `vsdbg` | Official Microsoft debugger; matches common C# VS Code `coreclr` launch configs; strong ecosystem recognition. | License/redistribution/standalone-use constraints are poor for a dap-cli built-in; provisioning commonly uses installer scripts rather than transparent pinned archives; may be better as user-provided custom adapter guidance than first-party setup. | Runner-up only if Rob explicitly accepts licensing/provisioning risk. |
...
- Is Rob comfortable choosing a non-Microsoft adapter because the Microsoft-owned alternative has license/provisioning risk?
